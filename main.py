...
def transcribe_audio(audio_url):
    ...
    try:
        ...
        # 新版API调用方式（2024年7月最新）
        response = dashscope.audio.asr.recognition(
        ...
        if response.status_code == HTTPStatus.OK:
            return response.output.sentence
...
def extract_summary(text):
    """提取摘要和关键词"""
    doc = nlp(text)
    keywords = [token.text for token in doc if token.pos_ in ["NOUN", "PROPN"]][:5]  # 取前5个名词
    summary = " ".join([sent.text for sent in doc.sents][:3])  # 取前3句
    return {
        "summary": summary,
...
if __name__ == "__main__":
    try:
        ...
        new_audio = notion.databases.query(
            db_id,
            filter={
                "and": [
